...
class Consign_stock_grid
{
  ...
  public function getConsignStockGrid($style_code)
	{
		$sc = '';
    ...
    if(!empty($style))
    {
      ...
      if( count($attrs) == 1  )
      {
        $sc .= $this->stockGridOneAttribute($style, $attrs[0]);
      }
      else if( count( $attrs ) == 2 )
      {
        $sc .= $this->stockGridTwoAttribute($style);
      }
  ...
  public function stockGridOneAttribute($style, $attr)
	{
		$sc 		= '';
    ...
    {
      $sc .= $i%2 == 0 ? '<tr>' : '';
      $stock	= $this->ci->stock_model->get_consignment_stock($item->code); //---- สต็อกทั้งหมดทุกคลัง
  ...
  public function stockGridTwoAttribute($style)
	{
