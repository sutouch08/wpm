  ...
  public function add_detail($order_code)
  {
    ...
    if(!empty($data))
    {
      foreach($data as $rs)
      {
        ...
        if( $qty > 0 && !empty($item))
        {
          $item->price = convertPrice($item->price, $order->DocRate);
          $item->cost = convertPrice($item->cost, $order->DocRate);
          ...
          if( $sumStock >= $qty OR $item->count_stock == 0 OR $auz == 1)
          {
            ...
            if( $this->orders_model->is_exists_detail($order_code, $item->code) === FALSE )
            {
              ...
              $ds['amount'] = convertPrice($discount['amount'], $order->DocRate);
              ...
              $arr = array(
                      ...
                      "totalFrgn"=> $order->DocRate > 0 ? ($line_total/$order->DocRate) : 0.00,
  ...
  public function update_order()
  {
    ...
    if($this->input->post('order_code'))
    {
      ...
      else
      {
        ...
        if($rs === TRUE)
        {
          if($recal == 1 OR $DocCur != $current_currency)
          {
            ...
            if(!empty($details))
            {
              foreach($details as $detail)
              {
                ...
                //---- คำนวณ ส่วนลดจากนโยบายส่วนลด
                $discount 	= $this->discount_model->get_item_discount($detail->order_code, $detail->product_code, $detail->price, $order->customer_code, $qty, $order->payment_code, $order->channels_code, $order->date_add);
