...
<div class="row">
  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding-5 table-responsive">
    <table class="table table-striped border-1" style=min-width:880px;>
      <tr>
				<?php if($this->pm->can_delete) : ?>
				<th class="fix-width-40 text-center"></th>
				<?php endif; ?>
        <th class="fix-width-40 text-center">#</th>
        <th class="fix-width-150">Date</th>
        <th class="fix-width-150">Document No.</th>
        <th class="fix-width-200">Item Code</th>
        <th class="fix-width-100 text-center">Qty.</th>
				<th class="fix-width-100 text-center">Box No.</th>
        <th class="min-width-100">State</th>
      </tr>
      <tbody>
    <?php if( !empty($data)) : ?>
    ...
    <?php foreach($data as $rs) : ?>
      <tr class="font-size-12" id="row-<?php echo $rs->id; ?>">
        ...
        <td class="text-center"><?php echo number($rs->qty); ?></td>
				<td class="text-center">
					<?php echo (empty($rs->box_id) ? "" : "Box No. ".$rs->box_no); ?>
				</td>
