...
<?php   foreach($list as $file)  : ?>
        <tr class="font-size-12" id="row-<?php echo $no; ?>">
          ...
          <td class="middle">
					...
					<?php endif; ?>
						<button type="button" class="btn btn-mini btn-primary" onclick="moveToList('<?php echo $file['name']; ?>', <?php echo $no; ?>)"><i class="fa fa-refresh"></i> Move to DO List</button>
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 table-responsive" style="max-height:70vh; overflow:auto;">
						<table class="table table-striped table-bordered tableFixHead" style="white-space: prewrap; font-size:12px; min-width:1270px;">
							<thead>
								<tr>
									<th class="fix-width-300 text-center fix-header">Error</th>
									<th class="fix-width-150 text-center fix-header">orderNumber</th>
									<th class="fix-width-100 text-center fix-header">date</th>
									<th class="fix-width-150 text-center fix-header">channels</th>
									<th class="fix-width-200 text-center fix-header">itemId</th>
									<th class="fix-width-60 text-center fix-header">qty</th>
									<th class="fix-width-60 text-center fix-header">price</th>
									<th class="fix-width-100 text-center fix-header">courier</th>
									<th class="fix-width-150 text-center fix-header">Shipping code</th>
								</tr>
							</thead>
							<tbody id="csv-detail">
...
<script>
	var HOME = BASE_URL + 'rest/V1/agx_delivery_error/';
	...
	function viewDetail(fileName) {
		...
		$.ajax({
			url:BASE_URL + "rest/V1/agx_delivery_error/get_detail",
			type:'POST',
			cache:false,
			data:{
				'fileName' : fileName
	...
	function doDelete(fileName, no){
		$.ajax({
			url:BASE_URL + "rest/V1/agx_delivery_error/delete",
			type:'POST',
			cache:false,
			data:{
				'fileName' : fileName
